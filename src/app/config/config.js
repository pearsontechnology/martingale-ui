...
import { fetch } from '@martingale/utils';
...
class Config extends React.Component {
  ...
  configurationLoaded(config = {}) {
    linkSpecs(
      ...
      this.trapError(
        ...
        this.trapError(
          linkSideNav,
          this.trapError((config) =>
            this.setState(Object.assign(config, { state: 'loaded' }))
          )
        )
      )
    );
  ...
  componentDidMount() {
    fetch(process.env.REACT_APP_CONFIG_FILE_LOCATION || '/config.json')
      .then((response) => {
